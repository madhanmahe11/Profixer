...
(function ($) {
  ...
  $('.submenu-item a').each(function () {
    ...
    if ($this.attr('href').indexOf(current) !== -1) {
      ...
      var parentCl = $this.parents('submenu a');
      parentCl.next('ul').attr('style', '');
      parentCl.next('ul').slideDown(350);
      parentCl.addClass('subdrop');
    }
  });
  ...
  if (!!hasTemp) {
    $('head').append(
      '<link id="tempSkin" rel="stylesheet" href="~/css/style-' +
        hasTemp +
        '.css">'
    );
    $('body')
